...
async def help_needed(message: types.Message):
    await message.answer("Я - бот, который может перенести стиль одной картинки на другую. "
                         ...
                         "Сеть я тренировала сама, поэтому результаты далеки от идеальных в силу слабого железа. Чтобы опробовать этот режим"
...
async def extra_case(message: types.Message):
    await message.answer("Ой, что-то не так, я не знаю такой команды. Если нужна помощь - вызови '/help'")
